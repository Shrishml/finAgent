...
async def admin_waitlist(request: Request):
    ...
    return JSONResponse(await list_waitlist_users())
...
async def admin_approve(request: Request):
    ...
    await set_user_access_status(uid, "active")
...
async def admin_create_codes(request: Request):
    _check_admin(request)
    body = await request.json()
    codes = await create_invite_codes(body.get("count", 5), body.get("max_uses", 1), body.get("note", ""))
...
async def admin_list_codes(request: Request):
    _check_admin(request)
    return JSONResponse(await list_invite_codes())
