...
async def _get_dev_user_id() -> int:
    global _DEV_USER_ID
    if _DEV_USER_ID is None:
        _DEV_USER_ID = await get_or_create_user("dev-user", "dev@arth.local", "Dev User", "")
...
async def get_user_id(request: Request) -> int | None:
    ...
    if _DEV_MODE and token == _DEV_TOKEN:
        return await _get_dev_user_id()
    user = get_session_user(token)
    if not user:
        return None
    return await get_or_create_user(user["google_id"], user["email"], user["name"], user["picture"])
